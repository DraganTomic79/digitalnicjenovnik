...
function createIconElement(item, type = 'kategorija') {
  ...
  // Ako nema URL-a ili nije http — prikaži FA ikonu (koristimo list-cat-icon, ne icon-preview)
...
/* KREIRANJE HTML */
function createEntityItemHTML(item, type, hasDrag = false) { const icon = createIconElement(item, type); const drag = hasDrag ? `<div class="${CONFIG.ui.classes.dragHandle}"><i class="${CONFIG.ui.icons.actions.grip}"></i></div>` : ''; const flags = `${item.specijalnaPonuda ? ' ⭐' : ''}${item.sadrziAlkohol ? ' 🔞' : ''}`; return `<div style="display:flex;align-items:center;flex:1;min-width:0;">${drag}${icon}<strong>${sanitizeText(item.naziv)}${flags}</strong></div><div class="${CONFIG.ui.classes.actions}"><button class="btn btn-sm btn-warning btn-edit" data-id="${item.id}"><i class="${CONFIG.ui.icons.actions.edit}"></i></button><button class="btn btn-sm btn-danger btn-delete" data-id="${item.id}"><i class="${CONFIG.ui.icons.actions.delete}"></i></button></div>`; }
function createPodkategorijaItemHTML(item, hasDrag = false) { const icon = createIconElement(item, 'podkategorija'); const drag = hasDrag ? `<div class="${CONFIG.ui.classes.dragHandle}"><i class="${CONFIG.ui.icons.actions.grip}"></i></div>` : ''; const flags = `${item.specijalnaPonuda ? ' ⭐' : ''}${item.sadrziAlkohol ? ' 🔞' : ''}`; return `<div style="display:flex;align-items:center;flex:1;min-width:0;">${drag}${icon}<div class="item-details"><strong style="display:block;margin-bottom:4px;">${sanitizeText(item.naziv)}${flags}</strong><small class="text-muted" style="display:block;">Kategorija: ${sanitizeText(item.kategorijaText)}</small></div></div><div class="${CONFIG.ui.classes.actions}"><button class="btn btn-sm btn-warning btn-edit" data-id="${item.id}"><i class="${CONFIG.ui.icons.actions.edit}"></i></button><button class="btn btn-sm btn-danger btn-delete" data-id="${item.id}"><i class="${CONFIG.ui.icons.actions.delete}"></i></button></div>`; }
...
export function ocistiFormuArtikal(elements) { clearFormFields(elements, ['naziv', 'cijena', 'podkategorijaSelect', 'opis', 'opis2', 'slika', 'slikaUrl']); }
